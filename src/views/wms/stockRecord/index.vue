    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['wms:stockRecord:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['wms:stockRecord:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
      ...
      <el-table v-loading="loading" border :data="stockRecordList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="变动类型(IN/OUT)" align="center" prop="changeType" />
        ...
        <el-table-column label="业务类型(STOCK_IN/STOCK_OUT)" align="center" prop="bizType" />
...
import { listStockRecord, getStockRecord, delStockRecord, addStockRecord, updateStockRecord } from '@/api/wms/stockRecord';
...
const data = reactive<PageData<StockRecordForm, StockRecordQuery>>({
  ...
  rules: {
    ...
    changeType: [
      { required: true, message: "变动类型(IN/OUT)不能为空", trigger: "change" }
    ],
    quantity: [
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('wms/stockRecord/export', {
    ...queryParams.value
